  ...
  switch (choice) {
    ...
    case `employeesManager`:
      prompt(prompts.viewMgrStaff).then((res) => {
        const managerIdIndex = staffList.indexOf(res.employeesManager);
    ...
    case `budget`:
      prompt(prompts.viewBudget).then((res) => {
        getQuery(`view`, queries.budget, res.departmentBudget);
      });
      break;
    default:
...
function add(choice) {
  switch (choice) {
    case `employee`:
      ...
      break;
    case `department`:
      prompt(prompts.addDepartment).then((res) => {
        getQuery(`add`, queries.addDepartment, { name: res.departmentName });
...
function remove(choice) {
  switch (choice) {
    case `employee`:
      prompt(prompts.removeEmployee).then((res) => {
        const employeeIdIndex = staffList.indexOf(res.removeEmployee);
        const employeeId = staffIds[employeeIdIndex];

        getQuery(`remove`, queries.removeEmployee, { id: employeeId });

        console.log(`--- Removed ${res.removeEmployee} from the database ---`);
      });
      break;
    case `department`:
      prompt(prompts.removeDepartment).then((res) => {
        getQuery(`remove`, queries.removeDepartment, {
          name: res.removeDepartment,
        });

        console.log(
          `--- Removed ${res.removeDepartment} as a Dept from the database. \nStaff has been terminated as well. ---`
        );
      });
      break;
    case `role`:
      prompt(prompts.removeRole).then((res) => {
        getQuery(`remove`, queries.removeRole, {
          title: res.removeRole,
        });

        console.log(
          `--- Removed ${res.removeRole} as a role from the database. \nStaff has been terminated as well.---`
        );
      });
      break;
    default:
