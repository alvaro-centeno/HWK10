  ...
  switch (choice) {
    ...
    case `staffDept`:
      prompt(prompts.viewDeptStaff).then((res) => {
        ...
        const departmentId = deptIds[departmentIdIndex];
  ...
  switch (choice) {
    ...
    case `department`:
      prompt(prompts.addDept).then((res) => {
        getQuery(`add`, queries.addDept, { name: res.deptName });

        console.log(`--- Added ${res.deptName} as a department! ---`);
      });
      break;
    case `role`:
